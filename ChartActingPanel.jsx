...
const ACTING_ORDER_CATALOG = [
  { code: "40020", key: "경혈 침술 1부위",        short: "침-1",    icon: "zap",         color: "#3A7065", allowedPhases: [1,2,3], default: true,  duration: 15, fee: 4920 },
  { code: "40021", key: "경혈 침술 2부위",        short: "침-2",    icon: "zap",         color: "#3A7065", allowedPhases: [1,2,3], default: true,  duration: 15, fee: 6210 },
  { code: "40022", key: "경혈 침술 3부위",        short: "침-3",    icon: "zap",         color: "#3A7065", allowedPhases: [2,3],   default: false, duration: 20, autoOnly: true, note: "자보 한정", fee: 7480 },
  { code: "40080", key: "침전기 자극술",          short: "전침",    icon: "activity",    color: "#4A7C6F", allowedPhases: [1,2,3], default: false, duration: 15, fee: 2860 },
  ...
  { code: "40310", key: "습식부항(자락관법)",     short: "습부항",  icon: "droplets",    color: "#B03028", allowedPhases: [1,2],   default: false, duration: 10, note: "급성·아급성기", fee: 6840 },
  { code: "40320", key: "약침술",                 short: "약침",    icon: "syringe",     color: "#2E5C52", allowedPhases: [1,2,3], default: false, duration: 10, fee: 11540 },
...
function AutoInsuranceBar({ injuryDate, today = new Date(), chunaUsed = 7, herbDaysUsed = 14, yakchimUsedWeek = 2, seupUsedWeek = 1 }) {
  ...
  return (
    <div style={{
      background: "linear-gradient(135deg, var(--ink-800) 0%, var(--jade-800) 100%)",
      color: "var(--stone-50)",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5, minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 14, fontWeight: 600, color: warn ? "var(--amber-200)" : "var(--stone-50)", lineHeight: 1, letterSpacing: "-0.01em", flexShrink: 0 }}>
    ...
    <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", padding: "14px 16px", marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: adding ? 12 : 0 }}>
          {dx.map(d => (
            <div key={d.code} style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: "var(--jade-700)", fontFamily: "var(--font-mono)", letterSpacing: "0.02em" }}>{d.code}</span>
      ...
      {!adding && aiSuggestions.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: "var(--jade-700)", letterSpacing: "0.04em", textTransform: "uppercase", fontFamily: "var(--font-sans)" }}>AI 추천</span>
          {aiSuggestions.map(s => (
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          ...
          <span style={{ fontSize: 18, fontWeight: 700, color: isAuto ? "var(--jade-700)" : "var(--cinnabar-700)", fontFamily: "var(--font-mono)", letterSpacing: "-0.01em" }}>
            ₩{fmt(patientPay)}
          </span>
        </div>
        {isAuto && (
    ...
    <div>
      ...
      {!showCatalog && phase && (
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 700, color: "var(--jade-700)", letterSpacing: "0.04em", textTransform: "uppercase", fontFamily: "var(--font-sans)" }}>AI 추천</span>
          {ACTING_ORDER_CATALOG
            .filter(c => c.allowedPhases.includes(phase) && !orders.find(o => o.code === c.code) && ["40080", "40300"].includes(c.code))
            .map(c => (
              <button key={c.code} onClick={() => addOrder(c)} style={{
