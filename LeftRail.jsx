    ...
    <div style={{ padding: "12px 14px 14px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
          <span style={{ fontFamily: "var(--font-serif)", fontSize: 16, fontWeight: 500, color: "var(--text-primary)", letterSpacing: "-0.01em" }}>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          ...
          <button onClick={() => { setView({ y: today.getFullYear(), m: today.getMonth() }); setSelected(today.getDate()); }}
            style={{ ...navBtn, fontSize: 10, fontWeight: 600, color: "var(--text-secondary)", padding: "0 8px", width: "auto" }}>오늘</button>
...
function TaskItem({ task, onToggle }) {
  const priColors = {
    ...
    low:  { dot: "var(--ink-300)",      text: "var(--text-muted)",   bg: "var(--stone-100)" },
  }[task.priority] || {};
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: `1.5px solid ${task.done ? "var(--jade-500)" : "var(--border-default)"}`,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 4, flexWrap: "wrap" }}>
          ...
          <span style={{
            ...
            color: task.due_far ? "var(--text-muted)" : "var(--text-secondary)",
            display: "inline-flex", alignItems: "center", gap: 3,
          }}>
...
function TasksPanel() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      ...
      {/* Filter tabs */}
      <div style={{ display: "flex", gap: 4, padding: "8px 12px", borderBottom: "1px solid var(--stone-200)" }}>
        {[{ k: "open", l: "진행 중" }, { k: "done", l: "완료" }, { k: "all", l: "전체" }].map(t => (
          <button key={t.k} onClick={() => setFilter(t.k)} style={{
            fontSize: 11, fontWeight: filter === t.k ? 600 : 400,
            padding: "3px 8px", borderRadius: "var(--radius-sm)",
            background: filter === t.k ? "var(--bg-surface)" : "transparent",
            border: filter === t.k ? "1px solid var(--border-subtle)" : "1px solid transparent",
            color: filter === t.k ? "var(--text-primary)" : "var(--text-muted)",
...
function LeftRail() {
  return (
    <div style={{
      ...
      <MiniCalendar />
      <div style={{ height: 8, background: "var(--stone-100)", borderTop: "1px solid var(--border-subtle)", borderBottom: "1px solid var(--border-subtle)" }} />
      <TasksPanel />
    </div>
  );
}
