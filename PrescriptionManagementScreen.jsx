...
// ── Workflow columns ────────────────────────────────────────────────
const RX_COLUMNS = [
  { key: "received",   label: "접수",     icon: "inbox",          bg: "var(--board-appt-bg)",  border: "var(--board-appt-border)",  next: "dispensing" },
  { key: "dispensing", label: "조제중",   icon: "flask-conical",  bg: "var(--board-wait-bg)",  border: "var(--board-wait-border)",  next: ["pickup", "shipping"] },
  { key: "pickup",     label: "픽업 대기", icon: "shopping-bag",  bg: "var(--board-treat-bg)", border: "var(--board-treat-border)", next: "completed" },
  { key: "shipping",   label: "배송중",   icon: "truck",          bg: "var(--board-pay-bg)",   border: "var(--board-pay-border)",   next: "completed" },
  { key: "completed",  label: "완료",     icon: "check-circle-2", bg: "var(--bg-raised)",      border: "var(--border-subtle)",      next: null },
];
        ...
        return (
          <div key={col.key}
            ...
            style={{
              ...
              background: col.bg,
              border: `1px solid ${isOver ? "var(--brand)" : col.border}`,
              borderRadius: "var(--radius-lg)", overflow: "hidden", minWidth: 0,
              boxShadow: isOver ? "0 0 0 2px var(--brand-muted)" : "none",
