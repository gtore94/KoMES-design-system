          ...
          {[
            ...
            <button key={o.key} onClick={() => set({ scope: o.key })}
              style={{
                ...
                color: s.scope === o.key ? "var(--text-brand)" : "var(--text-secondary)",
        ...
        {s.autoApprove && (
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <Icon name="alert-triangle" size={13} style={{ color: "var(--status-warning-text)", flexShrink: 0, marginTop: 2 }} />
              <div style={{ fontSize: 12, color: "var(--status-warning-text)", lineHeight: 1.5 }}>
  ...
  return (
    <div style={{ flex: "1 1 44%", background: "var(--bg-raised)", display: "flex", flexDirection: "column", minWidth: 0 }}>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", gap: 8 }}>
        <Icon name="eye" size={14} style={{ color: "var(--text-brand)" }} />
            ...
            {preview.groups.map(g => (
              <div key={g.supplier} style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 12, fontFamily: "var(--font-mono)", fontWeight: 600, color: "var(--text-brand)" }}>{won(g.total)}</span>
                </div>
                <div>
                  {g.items.map((i, idx) => (
                    <div key={i.id} style={{
  ...
  return (
    <div style={{ padding: "12px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 600, fontFamily: "var(--font-mono)", color: "var(--text-brand)" }}>
...
function RSToggle({ label, desc, value, onChange }) {
  return (
    <div onClick={() => onChange(!value)} style={{
      ...
      padding: "12px 14px",
      border: `1px solid ${value ? "var(--brand-muted)" : "var(--border-subtle)"}`,
...
function RSStatCard({ label, value, sub, icon, tone, small }) {
  const tones = {
    brand:   { bg: "var(--brand-subtle)",     border: "var(--brand-muted)",     fg: "var(--text-brand)" },
    warning: { bg: "var(--status-warning-bg)",   border: "var(--status-warning-border)",    fg: "var(--status-warning-text)" },
...
const chip = (sel) => ({
  ...
  color: sel ? "var(--text-brand)" : "var(--text-secondary)",
  borderRadius: "var(--radius-md)", cursor: "pointer",
  fontWeight: sel ? 600 : 400,
});
