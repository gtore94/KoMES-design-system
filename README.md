...
- **임상 데이터** — 상병·수가는 실제 고시 자료 기준 ([임상 데이터 기준](#임상-데이터-기준) 참고)
...
| | 진료 차트 | `chart` | `PatientChartScreen.jsx` + `ChartActingPanel.jsx`(상병·액팅 오더·청구), 처방 시 `PrescriptionScreen.jsx` |
...
상병명·시술명·수가는 임의 값이 아니라 아래 고시 자료에서 가져옵니다. 데이터를 새로 추가할 때도 같은 자료를 기준으로 하세요.

| 구분 | 출처 | 정의 위치 |
|------|------|-----------|
| 상병(KCD) | 심평원 상병마스터 (2025-09-30) | `ChartActingPanel.jsx` → `DIAGNOSIS_CATALOG` |
| 시술·수가 | 한방 수가리스트 (2025.11.1. 시행) — 한방병의원 단가 | `ChartActingPanel.jsx` → `ACTING_ORDER_CATALOG` |

### 상병

- **완전코드 + 주상병 사용 가능** 항목만 수록합니다. 마스터에서 `완전코드구분 = N`인 코드(`M54.5`, `M25.5`, `S33.5` 등)는 세분 코드가 필요해 그대로는 청구할 수 없으므로 5단위 코드(`M54.56`, `M25.51`, `S33.50`)를 씁니다.
- 각 항목은 표기용 `code`(점 구분)와 마스터 원본 `kcd`를 함께 가집니다. 상병 검색은 둘 다 매칭합니다 (`S33.50` / `S3350`).
- 마스터의 **한방 전용 상병**(양한방구분 `한방`, 296건) 중 `U30.3 항강`, `U23.8 비증`, `U22.2 화병`, `U23.4 중풍후유증`, `U32.7 산후풍`을 포함하며 `hanbang: true`로 표시되어 UI에 `한방` 뱃지가 붙습니다.

### 시술 · 수가

- 34개 항목을 8개 그룹(침술 · 부위 침술 · 분구침술 · 구술 · 부항 · 온냉경락 · 추나 · 검사·비급여)으로 묶어 **버튼 그리드**로 제공합니다 (진료 차트 우측 패널). 클릭 = 선택/해제.
- `covered: false`는 비급여(약침술 `49010`, 한방물리요법 `49020`)로 전액 본인부담 계산됩니다. 이 두 항목만 고시가가 없어 원내 설정 예시값을 씁니다.
- 자보 환자는 경과 단계(급성기 0–7일 / 아급성기 8–28일 / 만성기 29일+)에 따라 `allowedPhases`에 없는 항목이 취소선·비활성으로 잠깁니다.

### 명칭 규칙

전 화면에서 **고시 산정명칭**을 사용합니다. 관용어는 쓰지 않습니다.
...
| 전침 · 전기침술 | 침전기자극술 |
| 뜸 · 온뜸 | 간접구 |
| 부항 | 부항술(건식부항)-유관법 · 부항술(자락관법) |
| 추나 · 도수치료 | 추나요법-단순추나 · 복잡추나 |
| 약침 | 약침술 |
| 이학요법 | 한방물리요법 |

단, **수가 명칭이 아닌 것**은 그대로 둡니다 — 시설명(`침구실`, `약침실`, `추나실`), 비품명(`부항컵`, `뜸쑥`, `추나 베드`), 원내 비급여 상품명(`봉약침`, `미백 약침`), 직함(`추나전문의`).
...
- **화면 전용 헬퍼는 고유 접두사**를 붙입니다 — `NP*`(NewPatient), `Chart*`, `Rx*`, `Collect*`(수금), `Pkg*`/`pkg*`(패키지), `Pay*`(수납), `po*`(발주), `Form*`, `SR*`(NewStaff) 등.
- 흔히 겹치는 이름에 주의하세요. `DetailRow`·`KRW`·`addDays`처럼 화면마다 만들기 쉬운 헬퍼는 실제로 충돌한 적이 있습니다. 같은 이름이라도 파일마다 **동작이 달라**(`KRW` → `₩1,000` vs `1,000원`) 조용히 잘못된 값이 표시됩니다.
